      ...
      while (requesting) {
        ...
        switch(choice) {
          ...
          case 3:
            ...
            break;
          case 4:
            System.out.println("Choose charcount, enter 0 to count total chars in string or 1 to count specific char appearances:");
            String bool = scanner.nextLine();
            if (bool.equals("1")) {
              System.out.println("Choose specific char count, enter char to be counted:");
            ...
            json.put("count", sentence);
            break;
          case 5:
            System.out.println("Choose storyboard, enter 0 to view storyboard or 1 to add to storyboard: ");
            String view = scanner.nextLine();
            if (view.equals("1")) {
              System.out.println("Choose add to storyboard, enter name of user: ");
              String name = scanner.nextLine();
              System.out.println("Enter storyboard: ");
              String storyboard = scanner.nextLine();
              json.put("name", name);
              json.put("story", storyboard);
              json.put("view", false);
            }
            else {
              json.put("view", true);
        ...
        if (res.getBoolean("ok")){
          if (res.getString("type").equals("echo")) {
            ...
            JSONArray sb = res.getJSONArray("storyboard");
            if (res.getBoolean("view")) {
              for (int j = 0; j < sb.length(); j++) {
                System.out.println(sb.getJSONObject(j).get("story") + " " + sb.getJSONObject(j).get("name"));
              }
            } else {
              System.out.println(sb.getJSONObject(sb.length() - 1).get("story") + " " + sb.getJSONObject(sb.length() - 1).get("name"));
            }
          }else {
