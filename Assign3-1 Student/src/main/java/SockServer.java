...
public class SockServer {
  ...
  static JSONObject storyboard(JSONObject req) {
    ...
    res.put("ok", true);
    res.put("view", req.getBoolean("view"));
    try {
      if (!res.getBoolean("view")) {
        int i = arr.length();
        arr.put(i, req);
      }
      res.put("storyboard", arr);
    } catch (org.json.JSONException e) {
      res.put("ok", false);
      res.put("message", "array error message");
    }
    return res;
  }
  ...
  static JSONObject charCount(JSONObject req) {
    System.out.println("Count char request: " + req.toString());
    JSONObject res = new JSONObject();
    ...
    try {
      ...
      if (req.getBoolean("findchar")) {
        ...
        for (int i = 0; i < req.getString("count").length(); i++) {
          if (req.getString("count").charAt(i) == c) {
            total++;
          }
        }
      } else {
        total = req.getString("count").length();
      }
      res.put("result", total);
    } catch (org.json.JSONException e) {
      res.put("ok", false);
      res.put("message", "Error reason");
    }
    return res;
  }
